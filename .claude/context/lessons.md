...
- **Don't refactor the daemon-thread PG mirrors to call the pooled repos.** `paper_trade._mirror_to_postgres` (and the equity/alerts/crowding writers) open a FRESH `asyncpg.connect` inside a daemon thread on purpose: asyncpg pools are bound to the event loop that created them, and the shared pool from `init_pool()` lives on a different (or already-closed) loop. Calling `recommendations_repo.insert` (which uses `get_pool()`) from `asyncio.run()` in a daemon thread would get `None`/cross-loop errors. The pooled repos are for the READ path inside `init_pool/close_pool`-wrapped async MCP handlers, not fire-and-forget writes. *(Source: 2026-06-11 freshness audit flagged the 4 unwired repos as "pending migration to complete"; deeper check showed wiring them through the pool is unsafe - left dormant.)*

- **PG equity/recommendations reads use a session-derived tenant_hash, writes use `_LEGACY`.** `mcp_server._active_tenant_hash()` returns `sha1(session_id)` when WS-authed, else `sha1("legacy")`. `paper_trade`/equity mirrors always write under `_LEGACY`. So `get_live_kpis` (reads by active tenant) only sees mirrored rows when unauthenticated. Unifying this needs a single tenant source threaded through both write + read paths, plus a live Postgres to verify - don't ship it blind. `portfolio_equity` currently has NO live writer (only the one-shot `migrate_jsonl_to_postgres.py`), so `get_live_kpis` drawdown/Sharpe is starved on PG deployments - tracked follow-up. *(Source: 2026-06-11 freshness audit.)*
...
- **A "built" Postgres table is not a populated one - check there's a writer before reading it.** `signal_history.realized_return_*d` columns + `backfill_realized_returns` existed for months but NO caller ever computed/wrote the returns (`backfill_realized_returns` only SELECTs candidates). Every reader (weights_tuner attribution, continuous aggregate, calibration) silently ran on NULLs. Before trusting a DB-backed metric, confirm the write path runs. *(Source: 2026-06-11 signal_history PG port - JSONL `score_horizons` was the only thing actually computing returns; a prior session had already retreated calibration to JSONL for this reason.)*

- **Split-brain stores drift silently.** signal_history had TWO parallel stores: JSONL (interactive engine, nested `outcomes.hN`) and PG (scheduler, flat `realized_return_Nd`), with different horizon sets and different realized-return code. MCP read PG; the engine wrote JSONL → `get_signal_history` saw different data than was logged. When two writers feed "the same" concept, make one canonical and have the other mirror/fallback - never let both be authoritative. *(Source: 2026-06-11 port; fix = PG canonical, JSONL fallback only when no pool.)*

- **Don't gate behavior on infra presence when tenant-less callers depend on the old path.** Gating interactive `log_signal` on `not postgres_dsn` silently dropped JSONL audit for discovery/REST/skill_runner once PG was configured - PG only ever receives the scheduler's tenant-keyed rows, so those callers' signals vanished entirely. Use an explicit per-call flag (`log_jsonl=`) set by the writer that owns the canonical path, not a global config check. *(Source: 2026-06-11 review finding; fixed via keyword-only `log_jsonl`.)*

- **Per-call `init_pool()/close_pool()` is unsafe with a shared global pool.** A tool that closes the pool in its `finally` nulls `_pool` for any concurrent op still using it; with the long-running backfill in flight, its next `get_pool()` returns None and writes become silent no-ops. Pools are long-lived - init once, don't tear down per call. Also run idempotent startup migrations once per process (guard flag), not on every `init_pool`. *(Source: 2026-06-11 code review #1/#7.)*

- **Wealthsimple reports symbols BARE (no exchange prefix), with the exchange signalled only by `security.currency`.** TSX-listed names (XEQT, VFV) arrive as plain "XEQT" and 404 on yahoo, which expects "XEQT.TO". The old `.replace("TSX:","")` strip was a no-op (no prefix is ever present). Resolve via `KNOWN_CANADIAN`→`.TO`; do NOT blanket-append `.TO` on `currency==CAD` - dual-listed names like `GOLD` (CAD) have no `GOLD.TO` and would break a working US-listing fetch. *(Source: 2026-06-11 live WS payload inspection corrected the exchange-prefix assumption.)*
