...
1. Read `.claude/context/changes.md` — what's been built and when
2. Read `.claude/context/architecture.md` — data flow, API contracts, file index
3. Call `mcp__aifolimizer__get_profile` before any analysis — never hardcode account types or capital amounts

## What This Is
AI-powered investment advisor for a Canadian Wealthsimple user (age 32, growth + income + crypto investor).
Live portfolio data via local backend. AI analysis runs inside Claude Code / Claude Desktop (Pro plan) — no Anthropic API key needed.
...
```
MCP server (`mcp_server.py`) runs as a separate process managed by Claude Code — register once with:
```
...
Each skill: auto-triggers from description frontmatter, calls get_profile FIRST, then MCP tools, runs analysis in Claude's own context.

## Investor Profile (use as context, always verify with get_profile)

- Age: 32, Canadian resident
- Philosophy: growth stocks, index ETFs (XEQT/VFV), dividends, crypto exposure
- Risk appetite: mixed — conservative (bonds/GIC), moderate (index ETFs), aggressive (individual stocks, crypto)
- Time horizons: day trading + short-term (<3yr) + long-term (10yr+)
- Tax accounts: TFSA (gains tax-free), RRSP (tax-deferred), Non-Reg (50% capital gains inclusion)
...
- WS_EMAIL/WS_PASSWORD: local `.env` only, never committed, never logged, never sent to AI
- WS access token: server RAM only (Python dict), 8h TTL, never persisted
- `pii_filter.py` MUST run before every MCP tool response
- Account IDs, account numbers, email, full name: NEVER leave the server
...
- To scaffold a new skill: `python backend/scripts/build_skills.py --scaffold <tool_name>`
