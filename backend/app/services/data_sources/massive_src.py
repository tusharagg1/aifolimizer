...
def is_tsx(symbol: str) -> bool:
    ...
    upper = symbol.upper()
    return (
        any(upper.endswith(s) for s in _TSX_SUFFIXES)
        or upper in _KNOWN_CANADIAN
    )
...
class MassiveSource(DataSource):
    ...
    def _get_client(self):
        ...
        try:
            ...
            raise SourceUnavailable(
                f"massive package not installed: {e}"
            ) from e
            ...
            for agg in self._get_client().list_aggs(
                ...
                try:
                    ...
                    bars.append(PriceBar(
                        ...
                        adj_close=(
                            float(agg.vwap)
                            if getattr(agg, "vwap", None)
                            else float(agg.close)
                        ),
                        source=self.name,
                        as_of=time.time(),
                    ))
    ...
    def get_fundamentals(self, symbol: str) -> Fundamentals:
        ...
        raise SourceUnavailable(
            f"massive: fundamentals deferred to yfinance for {symbol}"
        )
