...
from app.models.portfolio import (  # noqa: E402
    PortfolioResponse, PortfolioSummary, Position,
)
...
def _synthetic_portfolio() -> PortfolioResponse:
    positions = [
        Position(symbol="AAPL", name="Apple", quantity=20,
                 currency="USD", book_cost=3500, book_cost_cad=4750,
                 market_value=4000, market_value_cad=5400,
                 day_change_pct=0.5, total_return_pct=14.3,
                 weight=18.0, asset_class="equity", sector="Technology"),
        Position(symbol="MSFT", name="Microsoft", quantity=10,
                 currency="USD", book_cost=3000, book_cost_cad=4080,
                 market_value=3500, market_value_cad=4750,
                 day_change_pct=0.3, total_return_pct=16.7,
                 weight=16.0, asset_class="equity", sector="Technology"),
        Position(symbol="JPM", name="JPMorgan", quantity=15,
                 currency="USD", book_cost=2500, book_cost_cad=3400,
                 market_value=2800, market_value_cad=3800,
                 day_change_pct=-0.2, total_return_pct=12.0,
                 weight=13.0, asset_class="equity", sector="Financials"),
        Position(symbol="JNJ", name="Johnson & Johnson", quantity=20,
                 currency="USD", book_cost=3000, book_cost_cad=4080,
                 market_value=2700, market_value_cad=3670,
                 day_change_pct=0.1, total_return_pct=-10.0,
                 weight=12.5, asset_class="equity", sector="Healthcare"),
        Position(symbol="XEQT.TO", name="iShares Core Equity ETF", quantity=100,
                 currency="CAD", book_cost=3000, book_cost_cad=3000,
                 market_value=3300, market_value_cad=3300,
                 day_change_pct=0.4, total_return_pct=10.0,
                 weight=11.0, asset_class="etf", sector=None),
        Position(symbol="VFV.TO", name="Vanguard S&P 500", quantity=30,
                 currency="CAD", book_cost=2500, book_cost_cad=2500,
                 market_value=2900, market_value_cad=2900,
                 day_change_pct=0.5, total_return_pct=16.0,
                 weight=9.5, asset_class="etf", sector=None),
        Position(symbol="PFE", name="Pfizer", quantity=80,
                 currency="USD", book_cost=2400, book_cost_cad=3260,
                 market_value=1800, market_value_cad=2450,
                 day_change_pct=-1.5, total_return_pct=-25.0,
                 weight=8.0, asset_class="equity", sector="Healthcare"),
        Position(symbol="XOM", name="Exxon Mobil", quantity=20,
                 currency="USD", book_cost=2000, book_cost_cad=2720,
                 market_value=2200, market_value_cad=2990,
                 day_change_pct=0.8, total_return_pct=10.0,
                 weight=8.0, asset_class="equity", sector="Energy"),
        Position(symbol="WMT", name="Walmart", quantity=15,
                 currency="USD", book_cost=1500, book_cost_cad=2040,
                 market_value=1750, market_value_cad=2380,
                 day_change_pct=0.2, total_return_pct=16.7,
                 weight=4.0, asset_class="equity", sector="Consumer Defensive"),
    ]
...
def check_skill_runner(portfolio, tenant_id: str) -> list[Check]:
    ...
    try:
        ...
        return [Check("skill_runner.run_all_skills executes").fail(
    ...
    if missing:
        checks.append(Check("all codified skills run").fail(
            f"missing: {sorted(missing)}",
        ))
    else:
        checks.append(Check("all codified skills run").ok(
    ...
    if errored:
        details = "\n".join(
            f"{s}: {out[s].get('error', '?')[:80]}" for s in errored
        )
    ...
    if len(composed) >= 3:
        checks.append(Check("daily-briefing composes >=3 skills").ok(
            f"composed_from={composed}",
        ))
    else:
        checks.append(Check("daily-briefing composes >=3 skills").fail(
    ...
    if len(files) >= len(got):
        checks.append(Check("snapshots persisted to tenant dir").ok(
            f"{len(files)} files in {snap_dir}",
        ))
    else:
        checks.append(Check("snapshots persisted to tenant dir").fail(
...
def check_tenant_isolation(portfolio) -> list[Check]:
    ...
    snap_a = skill_runner.read_snapshot("portfolio-health",
                                         tenant_id="tenant-A-smoke")
    snap_b = skill_runner.read_snapshot("portfolio-health",
                                         tenant_id="tenant-B-smoke")
    if snap_a and snap_b:
        return [Check("tenant isolation").ok(
            f"distinct dirs, both readable: {dir_a.name} vs {dir_b.name}",
        )]
    return [Check("tenant isolation").fail(
...
def check_recommendation_quality(
    ...
    try:
        ...
        return [Check("recommendations engine runs").fail(
            f"raised {type(e).__name__}: {e}",
        )], []
    ...
    actionable_pct = (
        (by_action.get("BUY", 0) + by_action.get("SELL", 0)
         + by_action.get("TRIM", 0) + by_action.get("ADD", 0))
        / len(recs)
    ) * 100

    dist = ", ".join(f"{a}={n}" for a, n in sorted(by_action.items()))
    if no_edge_pct > 90:
        checks.append(Check("NO_EDGE rate is sane (<=90%)").fail(
            f"{no_edge_pct:.0f}% NO_EDGE — engine too strict, no samples will accumulate. "
            f"Distribution: {dist}",
        ))
    elif no_edge_pct > 75:
        checks.append(Check("NO_EDGE rate is sane (<=90%)").ok(
            f"WARNING: {no_edge_pct:.0f}% NO_EDGE — high but acceptable. "
            f"Distribution: {dist}",
        ))
    else:
        checks.append(Check("NO_EDGE rate is sane (<=90%)").ok(
            f"{no_edge_pct:.0f}% NO_EDGE, {actionable_pct:.0f}% actionable. "
            f"Distribution: {dist}",
        ))

    # Every rec should have new contract fields populated
    sample = recs[0]
    required_fields = ["action", "score", "confidence", "current_price",
                       "reasons", "signal_quality"]
    missing = [f for f in required_fields if f not in sample]
    if missing:
        checks.append(Check("rec schema complete").fail(
            f"missing fields: {missing}",
        ))
    else:
        checks.append(Check("rec schema complete").ok(
...
def check_auto_log(portfolio, recs_from_prior_run: list[dict] | None) -> list[Check]:
    ...
    if not rec_file.exists():
        return [Check("auto-log writes to recommendations.jsonl").fail(
            "recommendations.jsonl does not exist",
        )]

    actionable = [r for r in (recs_from_prior_run or [])
                  if r.get("action") in ("BUY", "SELL", "TRIM", "ADD")]
    ...
    if not actionable:
        checks.append(Check("auto-log writes to recommendations.jsonl").ok(
        ...
        logged_syms = {row.get("ticker") for row in today_rows
                       if row.get("skill") == "recommendations_engine"}
        matched = actionable_syms & logged_syms
        if not matched:
            checks.append(Check("auto-log writes to recommendations.jsonl").fail(
                f"engine emitted actionable={sorted(actionable_syms)} but no "
                f"matching same-day row in jsonl",
            ))
        else:
            checks.append(Check("auto-log writes to recommendations.jsonl").ok(
    ...
    if today_rows:
        row = today_rows[-1]
        required = ["horizon_days", "benchmarks_entry", "confidence_source",
                    "model_version", "features"]
        missing = [f for f in required if f not in row]
        if missing:
            checks.append(Check("new schema on logged rec").fail(
                f"missing: {missing}",
            ))
        else:
            checks.append(Check("new schema on logged rec").ok(
...
def check_scoring() -> list[Check]:
    ...
    try:
        ...
        return [Check("score_recommendations runs").fail(
            f"raised {type(e).__name__}: {e}",
        )]

    if isinstance(result, dict) and "error" in result:
        # No data yet is fine on a fresh install
        return [Check("score_recommendations runs").ok(
...
def check_trust_helpers() -> list[Check]:
    ...
    for name, fn in (
        ("signal_decay_curve",
         lambda: signal_history.signal_decay_curve()),
        ("per_signal_source_attribution",
         lambda: signal_history.per_signal_source_attribution(21)),
        ("calibrate_confidence",
         lambda: signal_history.calibrate_confidence(21)),
    ):
        try:
            out = fn()
            ...
            checks.append(Check(f"{name} returns dict").fail(
                f"raised {type(e).__name__}: {e}",
            ))
    return checks
...
def main() -> int:
    ...
    print(f"  {len(portfolio.positions)} positions, "
          f"${portfolio.summary.total_value:,.0f} CAD")
