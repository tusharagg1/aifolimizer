...
def _stub_tech(stage: int = 2, rsi: float = 55, score: float = 0.0) -> dict:
    return {
        "stage": stage, "minervini_score": 4,
        "rsi": rsi, "current_price": 100.0, "sma_50": 95.0,
        "trend": "uptrend", "rsi_signal": "neutral",
    }
...
def _stub_position(symbol: str = "AAPL", weight: float = 3.0) -> dict:
    return {
        ...
        "currency": "USD", "asset_class": "stock",
    }
...
def test_skill_evidence_bullish_lifts_score():
    ...
    base = rec_svc._score_position(
        "AAPL", _stub_position(),
        _stub_tech(stage=2), _stub_fund(), {}, 0.2,
        skill_evidence=None,
    )
    ...
    assert bull["score"] > base["score"], (
        f"bullish skill evidence should lift score: "
        f"base={base['score']} bull={bull['score']}"
    )
...
def test_skill_score_clamped_to_two():
    ...
    rec = rec_svc._score_position(
        "AAPL", _stub_position(),
        _stub_tech(stage=2), _stub_fund(), {}, 0.0,
