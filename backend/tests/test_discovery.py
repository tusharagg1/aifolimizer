...
def _patch_rec_engine(monkeypatch, recs):
    ...
    def fake(positions):
        return [rec_map[p["symbol"]] for p in positions if p["symbol"] in rec_map]
...
def test_scan_excludes_held(monkeypatch):
    ...
    _patch_rec_engine(
        monkeypatch,
        [
            {"symbol": "AAPL", "action": "BUY", "score": 8.5, "confidence": "high", "kelly_pct": 5.0, "reasons": ["x"]},
            {"symbol": "NVDA", "action": "BUY", "score": 9.0, "confidence": "high", "kelly_pct": 7.0, "reasons": ["y"]},
        ],
    )
...
def test_scan_filters_below_min_score(monkeypatch):
    ...
    _patch_rec_engine(
        monkeypatch,
        [
            {"symbol": "WEAK", "action": "BUY", "score": 5.0, "confidence": "low", "reasons": []},
            {"symbol": "STRONG", "action": "BUY", "score": 8.5, "confidence": "high", "reasons": []},
        ],
    )
    picks = asyncio.run(
        d.scan_universe(
...
def test_scan_excludes_non_buy_actions(monkeypatch):
    ...
    _patch_rec_engine(
        ...
        [
            ...
            {"symbol": "BUYONE", "action": "BUY", "score": 7.5, "confidence": "med", "reasons": []},
        ],
    )
    picks = asyncio.run(
        d.scan_universe(
...
def test_top_n_picks_limits(monkeypatch):
    ...
    _patch_rec_engine(
        monkeypatch,
        [
            {"symbol": f"X{i}", "action": "BUY", "score": 8.0 + i * 0.1, "confidence": "med", "reasons": []}
            for i in range(10)
        ],
    )
    picks = asyncio.run(
